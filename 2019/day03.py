...
class Offset:
	def __init__(self, direction):
		if direction == WireDirection.UP:
			self.x = 0
			self.y = 1
		elif direction == WireDirection.RIGHT:
			self.x = 1
			self.y = 0
		elif direction == WireDirection.DOWN:
			self.x = 0
			self.y = -1
		elif direction == WireDirection.LEFT:
			self.x = -1
			self.y = 0
...
class WireSegment:
	...
	def __repr__(self):
		return '<WireSegment dir:{0}, dist:{1}>'.format(self.direction, self.distance)
...
def parse_route(rs):
	...
	for item in items:
		...
		if d == 'U':
			...
			print('Unrecognized direction {0}'.format(d))
			exit()
		distance = int(item[1:])
		movements.append(WireSegment(direction, distance))
	return movements

class Wire:
	def __init__(self, route_string):
		self.route = parse_route(route_string)
	def __repr__(self):
...
class Canvas:
	...
	def __repr__(self):
		return '<Canvas positions:{0}>'.format(self.wire_positions)

for i in range(len(test_data)):
	w1 = Wire(test_data[i][0])
	w2 = Wire(test_data[i][1])

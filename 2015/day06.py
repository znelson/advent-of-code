...
class LightGrid:
	...
	def _make_ranges(self, topleft, bottomright):
		...
		y_range = range(topleft.y, bottomright.y+1)
		return x_range, y_range

	def _update_rect(self, topleft, bottomright, updater):
		self._check_points(topleft, bottomright)

		x_range, y_range = self._make_ranges(topleft, bottomright)
		for y in y_range:
			for x in x_range:
				self.grid[y][x] = updater(self.grid[y][x])

	def turn_on_1(self, topleft, bottomright):
		def updater(item):
			return 1
		self._update_rect(topleft, bottomright, updater)

	def turn_off_1(self, topleft, bottomright):
		def updater(item):
			return 0
		self._update_rect(topleft, bottomright, updater)

	def toggle_1(self, topleft, bottomright):
		def updater(item):
			return 0 if item > 0 else 1
		self._update_rect(topleft, bottomright, updater)

	def turn_on_2(self, topleft, bottomright):
		def updater(item):
			return (item + 1)
		self._update_rect(topleft, bottomright, updater)

	def turn_off_2(self, topleft, bottomright):
		def updater(item):
			return (item - 1) if (item > 0) else 0
		self._update_rect(topleft, bottomright, updater)

	def toggle_2(self, topleft, bottomright):
		def updater(item):
			return (item + 2)
		self._update_rect(topleft, bottomright, updater)
...
for line in lines:
	...
	topleft, bottomright = Point(coords[0]), Point(coords[1])

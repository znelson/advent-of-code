...
class WireOperation:
	def __init__(self, rule):
		self.tokens = rule.split(' ')
		self.operator = None
		self.inputs = []
		self.output = None

		def _try_string_as_int(s):
			try:
				return int(s)
			except:
				return s
	...
	def is_valid(self):
		...
		if not self.output:
			return False
		if not self.operator and len(self.inputs) > 1:
			return False
		return True
...
lines = data.split('\n')
for rule in lines:
	wo = WireOperation(rule)
	operations.append(wo)
